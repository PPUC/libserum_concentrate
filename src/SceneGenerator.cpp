...
bool SceneGenerator::parseCSV(const std::string &csv_filename)
{
  ...
  while (std::getline(in_csv, line))
  {
    ...
    try
    {
      ...
      if (row.size() >= 6) data.repeat = (uint8_t)std::stoi(row[5]);
      if (row.size() >= 7) data.frameGroups = (uint8_t)std::stoi(row[6]) == 0 ? 1 : std::stoi(row[6]);
...
bool SceneGenerator::getSceneInfo(uint16_t sceneId, uint16_t &frameCount, uint16_t &durationPerFrame,
                                  bool &interruptable, bool &startImmediately, uint8_t &repeat, uint8_t &endFrame) const
{
                                           ...
                                           bool &startImmediately, uint8_t &repeat, uint8_t &endFrame) const
{
  ...
  {
    return getSceneInfo(m_autoStartSceneId, frameCount, durationPerFrame, interruptable, startImmediately, repeat,
                        endFrame);
  }
