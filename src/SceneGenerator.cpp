...
void SceneGenerator::setDepth(uint8_t depth)
{
   m_depth = depth;
   m_templateInitialized = false;
   initializeTemplate();
...
bool SceneGenerator::parseCSV(const std::string &csv_filename)
{
   ...
   while (std::getline(in_csv, line))
   {
      ...
      {
         SceneData data;
         data.sceneId = (uint16_t)std::stoi(row[0]);
         data.frameCount = (uint16_t)std::stoi(row[1]);
         data.durationPerFrame = (uint16_t)std::stoi(row[2]);
         if (row.size() >= 4)
            data.interruptable = (std::stoi(row[3]) == 1);
         if (row.size() >= 5)
            data.immediateStart = (std::stoi(row[4]) == 1);
         if (row.size() >= 6)
            data.repeat = (uint8_t)std::stoi(row[5]);
         if (row.size() >= 7)
            data.frameGroups = (uint8_t)std::stoi(row[6]) == 0 ? 1 : std::stoi(row[6]);
         if (row.size() >= 8)
...
bool SceneGenerator::generateDump(const std::string &dump_filename, int id)
{
   ...
   {
      if (id != -1 && scene.sceneId != (uint16_t)id)
      {
...
bool SceneGenerator::getSceneInfo(uint16_t sceneId, uint16_t &frameCount, uint16_t &durationPerFrame, bool &interruptable, bool &startImmediately, uint8_t &repeat, uint8_t &endFrame) const
{
   auto it = std::find_if(m_sceneData.begin(), m_sceneData.end(), [sceneId](const SceneData &data) { return data.sceneId == sceneId; });
...
bool SceneGenerator::getAutoStartSceneInfo(uint16_t &frameCount, uint16_t &durationPerFrame, bool &interruptable, bool &startImmediately, uint8_t &repeat, uint8_t &endFrame) const
{
   ...
   if (frameIndex == 0)
   {
      if (group == -1)
      {
         if (it->frameGroups > 1)
         {
            ...
            {
               it->currentGroup++;
               if (it->currentGroup > it->frameGroups)
                  it->currentGroup = 1;
            }
         }
         else
...
void SceneGenerator::renderChar(uint8_t *buffer, char c, uint8_t x, uint8_t y) const
{
...
void SceneGenerator::renderString(uint8_t *buffer, const std::string &str, uint8_t x, uint8_t y) const
{
   uint8_t currentX = x;
   for (char c : str)
   {
...
void SceneGenerator::saveToStream(LZ4Stream &stream) const
{
   uint32_t count = m_sceneData.size();
   stream.write(&count, sizeof(count));
   for (const auto &entry : m_sceneData)
   {
      // Write each field individually for better control
      stream.write(&entry.sceneId, sizeof(entry.sceneId));
      stream.write(&entry.frameCount, sizeof(entry.frameCount));
      stream.write(&entry.durationPerFrame, sizeof(entry.durationPerFrame));
      stream.write(&entry.interruptable, sizeof(entry.interruptable));
      stream.write(&entry.immediateStart, sizeof(entry.immediateStart));
      stream.write(&entry.repeat, sizeof(entry.repeat));
      stream.write(&entry.frameGroups, sizeof(entry.frameGroups));
      stream.write(&entry.currentGroup, sizeof(entry.currentGroup));
      stream.write(&entry.random, sizeof(entry.random));
...
void SceneGenerator::loadFromStream(LZ4Stream &stream)
{
   ...
   m_sceneData.reserve(count);

   // Read each SceneData entry
   for (uint32_t i = 0; i < count; ++i)
   {
      SceneData entry;

      // Read each field individually
      stream.read(&entry.sceneId, sizeof(entry.sceneId));
      stream.read(&entry.frameCount, sizeof(entry.frameCount));
      stream.read(&entry.durationPerFrame, sizeof(entry.durationPerFrame));
      stream.read(&entry.interruptable, sizeof(entry.interruptable));
      stream.read(&entry.immediateStart, sizeof(entry.immediateStart));
      stream.read(&entry.repeat, sizeof(entry.repeat));
      stream.read(&entry.frameGroups, sizeof(entry.frameGroups));
      stream.read(&entry.currentGroup, sizeof(entry.currentGroup));
      stream.read(&entry.random, sizeof(entry.random));
      stream.read(&entry.autoStart, sizeof(entry.autoStart));
      stream.read(&entry.endFrame, sizeof(entry.endFrame));
