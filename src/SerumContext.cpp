...
SerumContext::SerumContext()
{
   ...
   m_lastframeFound = static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count());
...
std::optional<std::string> SerumContext::FindCaseInsensitiveFile(const std::string& dir_path, const std::string& filename)
{
   ...
   try
   {
      for (const auto& entry : std::filesystem::directory_iterator(path_copy))
      {
         if (entry.is_regular_file())
         {
            ...
            if (ToLower(entry_filename) == lower_filename)
   ...
   if (frameId != IDENTIFY_NO_FRAME)
   {
      ...
      if (frameId < m_nframes && m_activeframes[m_lastfound][0] != 0)
      {
         ...
         for (uint32_t ti = 0; ti < MAX_COLOR_ROTATIONS; ti++)
         {
            ...
            if (m_colorrotnexttime[ti] == 0 || (m_colorshiftinittime[ti] + m_mySerum.rotations[ti * 3 + 2] * 10) <= currentTime)
            {
...
bool SerumContext::LoadRegularFile(const char* filename, uint8_t flags)
{
   ...
   if (ext && strcasecmp(ext, ".cROM") == 0)
   {
      ...
      if (strcpy_s(pathbuf, 4096, filename))
         return false;
   }
   ...
   if (!uncompressedCROM)
   {
      ...
      {
         if (strcpy_s(pathbuf, 4096, getenv("TMPDIR")))
            return false;
         size_t len = strlen(pathbuf);
         if (len > 0 && pathbuf[len - 1] != '/')
         {
            if (strcat_s(pathbuf, 4096, "/"))
               return false;
         }
      }
      ...
      if (!ExtractCRZ(filename, pathbuf, cromname, 4096))
         return false;
      if (strcat_s(pathbuf, 4096, cromname))
         return false;
   }
   ...
   if (fread(&m_ncompmasks, 4, 1, pfile) != 1 || fread(&m_nmovmasks, 4, 1, pfile) != 1 || fread(&m_nsprites, 4, 1, pfile) != 1)
   {
      fclose(pfile);
      return false;
   }
...
bool SerumContext::AllocateMemory()
{
   ...
   if (m_nsprites > 0)
   {
      ...
      if (!m_spritedescriptionso || !m_spritedescriptionsc || !m_spritedetdwords || !m_spritedetdwordpos || !m_spritedetareas)
      {
         return false;
      }
   }
...
bool SerumContext::ReadSeRumData(FILE* pfile, uint32_t sizeheader)
{
   ...
   if (fread(m_spritedetdwords, sizeof(uint32_t), m_nsprites * MAX_SPRITE_DETECT_AREAS, pfile) != m_nsprites * MAX_SPRITE_DETECT_AREAS
      || fread(m_spritedetdwordpos, sizeof(uint16_t), m_nsprites * MAX_SPRITE_DETECT_AREAS, pfile) != m_nsprites * MAX_SPRITE_DETECT_AREAS
      || fread(m_spritedetareas, sizeof(uint16_t), m_nsprites * 4 * MAX_SPRITE_DETECT_AREAS, pfile) != m_nsprites * 4 * MAX_SPRITE_DETECT_AREAS)
   {
      return false;
   }
...
bool SerumContext::LoadConcentrateFile(const char* filename, uint8_t flags)
{
   ...
   if (!stream.read(&m_serumVersion, sizeof(uint8_t)) || !stream.read(&m_fwidth, sizeof(uint32_t)) || !stream.read(&m_fheight, sizeof(uint32_t)) || !stream.read(&m_fwidthx, sizeof(uint32_t))
      || !stream.read(&m_fheightx, sizeof(uint32_t)) || !stream.read(&m_nframes, sizeof(uint32_t)) || !stream.read(&m_nocolors, sizeof(uint32_t))
      || !stream.read(&m_nccolors, sizeof(uint32_t)) || !stream.read(&m_ncompmasks, sizeof(uint32_t)) || !stream.read(&m_nsprites, sizeof(uint32_t))
   ...
   if (!m_isextrarequested)
      m_isextrabackground.clearIndex();
   ...
   if (m_nsprites > 0)
   {
      ...
      if (m_serumVersion == SERUM_V2)
      {
         ...
         if (!stream.read(m_spriteoriginal, m_nsprites * MAX_SPRITE_WIDTH * MAX_SPRITE_HEIGHT) || !stream.read(m_sprshapemode, m_nsprites))
         {
            FreeResources();
            fclose(pfile);
            return false;
      ...
      else if (m_serumVersion == SERUM_V1)
      {
         ...
         if (!stream.read(m_spritedescriptionso, m_nsprites * MAX_SPRITE_SIZE * MAX_SPRITE_SIZE) || !stream.read(m_spritedescriptionsc, m_nsprites * MAX_SPRITE_SIZE * MAX_SPRITE_SIZE))
         {
            FreeResources();
            fclose(pfile);
            return false;
...
bool SerumContext::SaveConcentrateFile(const char* filename)
{
   ...
   uint16_t concentrateFileVersion = 1;
   if (!stream.write(&concentrateFileVersion, sizeof(uint16_t)) || !stream.write(&m_serumVersion, sizeof(uint8_t)) || !stream.write(&m_fwidth, sizeof(uint32_t))
      || !stream.write(&m_fheight, sizeof(uint32_t)) || !stream.write(&m_fwidthx, sizeof(uint32_t)) || !stream.write(&m_fheightx, sizeof(uint32_t))
      || !stream.write(&m_nframes, sizeof(uint32_t)) || !stream.write(&m_nocolors, sizeof(uint32_t)) || !stream.write(&m_nccolors, sizeof(uint32_t))
      || !stream.write(&m_ncompmasks, sizeof(uint32_t)) || !stream.write(&m_nsprites, sizeof(uint32_t)) || !stream.write(&m_nbackgrounds, sizeof(uint16_t)))
   {
      fclose(pfile);
      return false;
   }
   ...
   if (m_nsprites > 0)
   {
      ...
      if (m_spritedetdwordpos && !stream.write(m_spritedetdwordpos, sizeof(uint16_t) * m_nsprites * MAX_SPRITE_DETECT_AREAS))
      {
         fclose(pfile);
         return false;
      }
      if (m_spritedetareas && !stream.write(m_spritedetareas, sizeof(uint16_t) * m_nsprites * 4 * MAX_SPRITE_DETECT_AREAS))
      {
         fclose(pfile);
         return false;
      }
...
bool SerumContext::LoadFilev2(const char* filename, uint8_t flags, bool uncompressed, uint32_t sizeheader)
{
   ...
   if (fread(&fwidth, 4, 1, pfile) != 1 || fread(&fheight, 4, 1, pfile) != 1 || fread(&fwidthx, 4, 1, pfile) != 1 || fread(&fheightx, 4, 1, pfile) != 1)
   {
      fclose(pfile);
      return false;
   }
   ...
   if (fread(&m_ncompmasks, 4, 1, pfile) != 1 || fread(&m_nsprites, 4, 1, pfile) != 1 || fread(&m_nbackgrounds, 2, 1, pfile) != 1)
   {
      fclose(pfile);
      return false;
   }
   ...
   if (flags & FLAG_REQUEST_32P_FRAMES)
   {
      ...
      if (!m_mySerum.frame32 || !m_mySerum.rotations32 || !m_mySerum.rotationsinframe32 || (flags & FLAG_REQUEST_FILL_MODIFIED_ELEMENTS && !m_mySerum.modifiedelements32))
      {
         FreeResources();
         fclose(pfile);
         return false;
   ...
   if (flags & FLAG_REQUEST_64P_FRAMES)
   {
      ...
      if (!m_mySerum.frame64 || !m_mySerum.rotations64 || !m_mySerum.rotationsinframe64 || (flags & FLAG_REQUEST_FILL_MODIFIED_ELEMENTS && !m_mySerum.modifiedelements64))
      {
         FreeResources();
         fclose(pfile);
         return false;
   ...
   if (m_nsprites > 0 && fread(m_spriteoriginal, 1, m_nsprites * MAX_SPRITE_WIDTH * MAX_SPRITE_HEIGHT, pfile) != m_nsprites * MAX_SPRITE_WIDTH * MAX_SPRITE_HEIGHT)
   {
      FreeResources();
      fclose(pfile);
      return false;
   ...
   if (m_nsprites > 0)
   {
      if (fread(m_spritedetdwords, 4, m_nsprites * MAX_SPRITE_DETECT_AREAS, pfile) != m_nsprites * MAX_SPRITE_DETECT_AREAS
         || fread(m_spritedetdwordpos, 2, m_nsprites * MAX_SPRITE_DETECT_AREAS, pfile) != m_nsprites * MAX_SPRITE_DETECT_AREAS
         || fread(m_spritedetareas, 2, m_nsprites * 4 * MAX_SPRITE_DETECT_AREAS, pfile) != m_nsprites * 4 * MAX_SPRITE_DETECT_AREAS)
      {
         FreeResources();
         fclose(pfile);
         return false;
   ...
   if (m_nsprites > 0)
      memset(m_sprshapemode, 0, m_nsprites);
         ...
         if (m_nsprites > 0)
         {
            ...
            for (uint32_t i = 0; i < m_nsprites; i++)
            {
               if (m_sprshapemode[i] > 0)
               {
                  ...
                  {
                     if (m_spriteoriginal[j] > 0 && m_spriteoriginal[j] != 255)
                        m_spriteoriginal[j] = 1;
                  }
               }
            }
         }
