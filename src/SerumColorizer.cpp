...
Serum::Serum()
   : m_context(std::make_unique<SerumContext>())
   , m_loaded(false)
{
}
...
void Serum::SetIgnoreUnknownFramesTimeout(uint16_t milliseconds) { m_context->SetIgnoreUnknownFramesTimeout(milliseconds); }

void Serum::SetMaximumUnknownFramesToSkip(uint8_t maximum) { m_context->SetMaximumUnknownFramesToSkip(maximum); }

void Serum::SetStandardPalette(const uint8_t* palette, int bitDepth) { m_context->SetStandardPalette(palette, bitDepth); }
...
{
   if (!m_context || !m_context->m_sceneGenerator)
      return false;
...
bool Serum::Scene_GetInfo(uint16_t sceneId, uint16_t* frameCount, uint16_t* durationPerFrame, bool* interruptable, bool* startImmediately, uint8_t* repeat, uint8_t* endFrame)
{
   if (!m_context || !m_context->m_sceneGenerator)
      return false;
   return m_context->GetSceneGenerator().getSceneInfo(sceneId, *frameCount, *durationPerFrame, *interruptable, *startImmediately, *repeat, *endFrame);
}
...
{
   if (m_context && m_context->m_sceneGenerator)
      m_context->GetSceneGenerator().setDepth(depth);
}
...
{
   if (m_context && m_context->m_sceneGenerator)
      m_context->GetSceneGenerator().Reset();
}
