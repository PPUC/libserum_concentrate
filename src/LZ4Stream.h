...
class LZ4Stream {
 public:
  ...
  LZ4Stream(FILE *file, bool writing,
            int compressionLevel = 12)  // Default is max compression
  ...
  bool write(const void *data, size_t size) {
    ...
    while (remaining > 0) {
      ...
      int compressedSize = LZ4_compress_HC_continue(
          ctx_hc_, src, (char *)outBuf_.data(), chunk, outBuf_.size());
  ...
  bool read(void *data, size_t size) {
    ...
    while (remaining > 0) {
      ...
      int decompressedSize = LZ4_decompress_safe_continue(
          dctx_, (char *)inBuf_.data(), dst, compressedSize, chunk);
