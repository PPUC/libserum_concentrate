...
{
  if (!g_serum) g_serum = std::make_unique<Serum::Serum>();
  return *g_serum;
}
...
{
  return GetSerum().Scene_GenerateDump(dump_filename, id);
}
...
{
  return GetSerum().Scene_GetInfo(sceneId, frameCount, durationPerFrame, interruptable, startImmediately, repeat,
                                  endFrame);
}
SERUM_API bool Serum_Scene_GenerateFrame(uint16_t sceneId, uint16_t frameIndex, uint8_t* buffer, int group)
{
  return GetSerum().Scene_GenerateFrame(sceneId, frameIndex, buffer, group);
}
