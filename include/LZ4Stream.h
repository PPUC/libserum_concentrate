 ...
 public:
  ...
  bool write(const void *data, size_t size)
  {
    ...
    while (remaining > 0)
    {
      ...
      int compressedSize = LZ4_compress_HC_continue(ctx_hc_, src, (char *)outBuf_.data(), chunk, outBuf_.size());

      if (compressedSize <= 0) return false;

      // Write compressed chunk size and data
      uint32_t chunkSize = compressedSize;
      if (fwrite(&chunkSize, sizeof(chunkSize), 1, file_) != 1) return false;
      if (fwrite(outBuf_.data(), 1, compressedSize, file_) != compressedSize) return false;
  ...
  bool read(void *data, size_t size)
  {
    ...
    while (remaining > 0)
    {
      ...
      if (fread(&compressedSize, sizeof(compressedSize), 1, file_) != 1) return false;

      if (fread(inBuf_.data(), 1, compressedSize, file_) != compressedSize) return false;

      size_t chunk = (std::min)(remaining, CHUNK_SIZE);
      int decompressedSize = LZ4_decompress_safe_continue(dctx_, (char *)inBuf_.data(), dst, compressedSize, chunk);
