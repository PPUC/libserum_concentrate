 ...
 *  @param flags (only needed for v2 files): this can be a combination of: FLAG_REQUEST_32P_FRAMES (if you want the
 * 32-pixel-high frame to be returned if available) / FLAG_REQUEST_64P_FRAMES (same for 64-pixel-high frame) /
 ...
 *
 * @return Either return IDENTIFY_NO_FRAME if no frame in the Serum file matches or, if a frame was identified, 0 if it
 * has no rotation or a value giving the time in milliseconds before the first color rotation
 */
SERUM_API uint32_t Serum_Colorize(uint8_t* frame);

/** @brief Perform the color rotations of the current frame. For v1, it modifies "palette", for v2, it modifies
 * "frame32" and/or "frame64"
 *
 * @return The time in milliseconds before the next rotation in the low word. If there was a rotation in the 32P frame,
 * the first bit of the high word is set (0x10000) and if there was a rotation in the 64P frame, the second bit of the
 ...
 */
SERUM_API bool Serum_Scene_GetInfo(uint16_t sceneId, uint16_t* frameCount, uint16_t* durationPerFrame,
                                   bool* interruptable, bool* startImmediately, uint8_t* repeat, uint8_t* endFrame);
