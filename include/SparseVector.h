...
class SparseVector
{
 ...
 public:
  ...
  SparseVector(T noDataSignature) : useIndex(false), useCompression(false) { noData.resize(1, noDataSignature); }
    ...
    else
    {
      ...
      if (useCompression)
      {
        ...
        int decompressedSize = LZ4_decompress_safe(
            reinterpret_cast<const char *>(compressed.data()), reinterpret_cast<char *>(lastDecompressed.data()),
            static_cast<int>(compressed.size()), static_cast<int>(elementSize * sizeof(T)));
  ...
  {
    if (useIndex) return elementId < index.size() && !index[elementId].empty() && index[elementId][0] != noData[0];
    ...
    if (parent == nullptr || parent->hasData(elementId))
    {
      if (memcmp(values, noData.data(), elementSize * sizeof(T)) != 0)
      {
        if (useCompression)
        {
          ...
          int compressedSize =
              LZ4_compress_HC(reinterpret_cast<const char *>(values), reinterpret_cast<char *>(compBuffer.data()),
                              static_cast<int>(elementSize * sizeof(T)), static_cast<int>(maxCompressedSize),
                              12  // max compression level
              );

          if (compressedSize > 0)
          {
            data[elementId].assign(compBuffer.begin(), compBuffer.begin() + compressedSize);
          }
        }
        else
        {
